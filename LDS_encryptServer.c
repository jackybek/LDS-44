...
int encryptServer(UA_Server *uaLDSServer, UA_ServerConfig *config)
{
...
printf("LDS_encryptServer.c : after UA_ServerConfig_setMinimal \n");
	...
	// This section uses self-signed certificate generated using generateSSCert() function
	{
		...
		status = generateSSCert(uaLDSServer, config,
				trustList, trustListSize,
                                issuerList, issuerListSize,
                                revocationList, revocationListSize,
				&derPrivKey, &derCert);
...
int generateSSCert(UA_Server *uaLDSServer, UA_ServerConfig *config,
		   ...
		   UA_ByteString *derPrivKey, UA_ByteString *derCert)
{
	...
	*/
	...
	//UA_ServerConfig *config = UA_Server_getConfig(uaLDSServer);
        //if (!config)
        //     UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"LDS_encryptServer.c : Error getting pointer to config");
	UA_ServerConfig_setMinimal(config, 4840, NULL);
	status = UA_ServerConfig_setDefaultWithSecurityPolicies(config, 4840,
							derCert, derPrivKey,
							trustList, trustListSize,
